              ...
              tick_t now)              /* time of access */
{
  if (cache_dl3)
  {
    return cache_access(cache_dl3, cmd, baddr, NULL, bsize, now, NULL, NULL);
  }
  else
  {
    /* access main memory, which is always done in the main simulator loop */
...
void sim_reg_options(struct opt_odb_t *odb) /* options database */
{
  opt_reg_header(odb,
      ...
      else
      {
        if( sscanf(cache_dl3_opt, "%[^:]:%d:%d:%d:%c",
          ...
          fatal("bad l3 D-cache parms: "
                "<name>:<nsets>:<bsize>:<assoc>:<repl>");
        cache_dl3 = cache_create(name, nsets, bsize, FALSE, 0, assoc, cache_char2policy(c), dl3_access_fn, 1);
      }
    }
  }
    ...
    else
    {
      ...
      else
      {
        if( sscanf(cache_il3_opt, "%[^:]:%d:%d:%d:%c",
                  name, &nsets, &bsize, &assoc, &c) != 5)
          fatal("bad l3 I-cache params: "
                "<name>:<nsets>:<bsize>:<assoc>:<repl>");
        else
        {
          cache_il3 = cache_create(name, nsets, bsize, FALSE, 0, assoc, cache_char2policy(c), il3_access_fn, 1);
        }
      }
    }
  }
